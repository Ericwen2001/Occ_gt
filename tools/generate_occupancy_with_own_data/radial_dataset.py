...
class RADIal(Dataset):
    ...
    def __init__(self, root_dir,difficult=False):
        ...
        self.labels = pd.read_csv(os.path.join(root_dir,'radial_bbox_labels.csv')).to_numpy()
        ...
        self.unique_seqs = np.unique(self.labels[:,14])
        self.label_dict = {}
    ...
    def __getitem__(self, index):
        ...
        # Get the sample id
        sample_id = int(self.sample_keys[index])
        ...
        # use the reannotated labels from EchoFusion
        # Labels contains following parameters:
        # numSample,radar_X_m,radar_Y_m,radar_Z_m,dim_X_m,dim_Y_m,dim_Z_m,rotation_y,class
        box_labels = box_labels[:, 1:].astype(np.float32)
