...
def main():
    ...
    for split in splits:
        ...
        for clip in raw_clips:
            ...
            path = os.path.join(root_path, split, clip, 'semantic_occupancy_gt_fov')
            vis_path = '/mnt/data/DataSet/viz/gt_fov/{}'.format(clip)
            ...
            for i in range(0, len_sequence):
                fov_voxels = np.load(os.path.join(path, 'occupancy_gt_with_semantic_fov{}.npy'.format(i)))
                
                # Filter class indices
                fov_voxels[:, 3] = np.where(fov_voxels[:, 3] > 2, 2, fov_voxels[:, 3])
                ...
                if plt_plot_fov is None:
                    ...
                    mlab.view(azimuth=0, elevation=-65, distance=100, focalpoint=(50,65,0))
                else:
                    plt_plot_fov.mlab_source.reset(
                        x=fov_voxels[:, 0],
                        y=fov_voxels[:, 1],
                        z=fov_voxels[:, 2],
                        scalars=fov_voxels[:, 3]
                    )
